...
# Build UI - simplified version
with gr.Blocks(title="StartupOps AI Simulator") as demo:
    gr.Markdown("# StartupOps AI Simulator")
    gr.Markdown("A deterministic RL environment simulating startup operations.")
    ...
    with gr.Tab("Auto Mode"):
        ...
        with gr.Row():
            ...
            with gr.Column():
                ...
                auto_logs = gr.Textbox(label="Logs", lines=10)
                auto_scores = gr.Textbox(label="Scores", lines=10)
        ...
        with gr.Row():
            with gr.Column():
                ...
                manual_tasks = gr.Textbox(
                    ...
                    lines=5,
                    placeholder="Fix bug, 4, 2, high\nUpdate docs, 2, 5, medium",
                )
                seed_manual = gr.Number(value=42, label="Seed")
                steps_manual = gr.Slider(10, 100, value=50, step=10, label="Max Steps")
                run_manual_btn = gr.Button("Run Simulation")

            with gr.Column():
                manual_summary = gr.Textbox(label="Summary", lines=10)
                manual_logs = gr.Textbox(label="Logs", lines=10)
                manual_scores = gr.Textbox(label="Scores", lines=10)
    ...
    with gr.Tab("About"):
        ...
        - **reply_email**: Removes email, boosts satisfaction
        - **ignore_email**: Penalty if urgency=high
        - **assign_task**: Consumes team_hours
